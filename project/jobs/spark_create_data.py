...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_source",
        help="The URI for your input network graph data folder, like an S3 bucket location.",
    )
    parser.add_argument(
        "--data_output",
        help="The URI for your output data, like an S3 bucket location.",
    )
    args = parser.parse_args()
    input_path = args.data_source
    output_path = args.data_output
    G = read_graph(input_path)
    df = create_test_links_spark(G, PERCENTAGE)
    df.write.csv(output_path + input_path.split("/")[-1], header=True)
